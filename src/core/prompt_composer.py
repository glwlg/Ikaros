...
class PromptComposer:
    ...
    def _build_ikaros_session_context_contract() -> str:
        return (
            "【当前会话上下文约束】\n"
            "- 私聊会话每轮会注入刷新后的 `【会话记忆种子】`（核心长期事实）；"
            "可能还有按本轮问题检索的 `【相关记忆】`、任务 `【工作状态】`、以及长会话 `【会话压缩摘要】`。\n"
            "- 组装顺序：核心记忆 → 工作状态 → 压缩摘要 → 相关记忆 → 最近对话。"
            "先使用这些会话内上下文；默认不要再调用 `read` 直接读取 MEMORY.md 或日记文件。\n"
            "- 只有在用户明确要求查看/修正记忆，或专门做记忆维护时，才深入读写记忆存储。\n"
            "- 若当前用户消息与任何记忆冲突，以当前用户最新明确表达为准。\n"
