...
@pytest.mark.asyncio
async def test_news_article_writer_uses_local_material_without_search(monkeypatch, tmp_path: Path):
    module = _load_module()
...
@pytest.mark.asyncio
async def test_news_article_writer_returns_recoverable_error_for_missing_local_material():
    module = _load_module()
    ctx = SimpleNamespace(
        message=SimpleNamespace(
            text="",
...
@pytest.mark.asyncio
async def test_news_article_writer_generates_xiaohongshu_draft_files(
    monkeypatch,
):
    module = _load_module()
    monkeypatch.setattr(module, "get_current_model", lambda: "demo/current-model")
...
@pytest.mark.asyncio
async def test_news_article_writer_xiaohongshu_publish_preflight_fails_before_generation(
    monkeypatch,
):
    module = _load_module()
    monkeypatch.setattr(
...
def test_news_article_writer_params_from_args_include_source_paths():
    module = _load_module()
...
def test_news_article_writer_params_from_args_include_publish_channels():
    module = _load_module()
    parser = module._build_parser()
