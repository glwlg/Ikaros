...
from api.api.endpoints.skills import SkillCreateRequest
...
async def test_create_skill_rejects_duplicate(registry):
    await skills_endpoint.create_skill(SkillCreateRequest(name="dup_skill"))
...
async def test_create_skill_rejects_invalid_name(registry):
    with pytest.raises(HTTPException) as exc:
        await skills_endpoint.create_skill(SkillCreateRequest(name="bad/name"))
...
async def test_import_skill_from_markdown(registry):
    ...
    result = await skills_endpoint.import_skill(_upload(payload, "whatever.md"))
...
async def test_import_skill_from_zip_extracts_full_directory(registry):
    ...
    result = await skills_endpoint.import_skill(_upload(payload, "demo.zip"))
...
async def test_import_zip_requires_skill_md(registry):
    ...
    with pytest.raises(HTTPException) as exc:
        await skills_endpoint.import_skill(_upload(payload, "demo.zip"))
...
@pytest.mark.asyncio
async def test_delete_skill_removes_learned_skill(registry):
    await skills_endpoint.create_skill(SkillCreateRequest(name="to_delete"))
    assert registry.get_skill("to_delete")

    result = await skills_endpoint.delete_skill("to_delete")

    assert result == {"name": "to_delete", "deleted": True}
...
async def test_delete_skill_rejects_builtin(registry, tmp_path):
    ...
    with pytest.raises(HTTPException) as exc:
        await skills_endpoint.delete_skill("sys_skill")
...
async def test_delete_skill_missing_returns_404(registry):
    with pytest.raises(HTTPException) as exc:
        await skills_endpoint.delete_skill("missing_skill")
